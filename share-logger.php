...
class ShareLogger extends ShareErrors{
    ...
    public function createTable() {
        ...
        $resultDrop = $db->query();
        if(!$resultDrop){
            $this->setError(self::$ERROR_TYPE_ERROR, $this->_plgLangPrefix.'DROP_TABLE_FAILED');
            return false;
        }
        ...
        $this->setError(self::$ERROR_TYPE_ERROR, $this->_plgLangPrefix.'CREATE_TABLE_FAILED');
        return FALSE;
    }
     ...
     */
    public function log() {
        $itemId = $this->_JArticle->id;
        $catId = $this->_JArticle->catid;
        $option = JRequest::getCmd('option');
        $now = date("Y-m-d H:i:s");
        $language = $this->_JArticle->language;
        $db =& JFactory::getDbo();
        ...
        $this->setError(self::$ERROR_TYPE_ERROR, $this->_plgLangPrefix.'SAVE_LOG_FAILED');
        return FALSE;
    }
    ...
    public function unlog() {
        ...
        $query->where('article_id = '.$db->quote((int)$this->_JArticle->id));
        ...
        $this->setError(self::$ERROR_TYPE_ERROR, $this->_plgLangPrefix.'UNLOG_FAILED');
        return FALSE;
    }
    ...
    public function isLogged(){
        ...
        if($isLogged){
            $this->setError(self::$ERROR_TYPE_NOTICE, $this->_plgLangPrefix.'ALREADYSENT');
            return TRUE;
        }
        return FALSE;
    }
